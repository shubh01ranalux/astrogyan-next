...
import {
  calculateLuckyDatesReport,
  type LuckyDatesReport,
} from "@/lib/calculators/numerology/lucky-dates";
...
export default function LuckyDatesCalculator() {
  const supabase = createClient();

  const [result, setResult] = useState<LuckyDatesReport | null>(null);
  ...
  async function handleCalculate(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const report = calculateLuckyDatesReport(dobValue);
      setResult(report);

      await saveCalculatorLead({
        supabase,
        sourceSlug: "lucky-dates-calculator",
        sourceTitle: "Lucky Dates Calculator",
        sourceUrl: "/free-tools/lucky-dates-calculator",
        fullName: lead.fullName,
        phone: lead.phone,
        email: lead.email,
        gender: lead.gender,
        dateOfBirth: dobValue,
        leadIntent: "Generated Lucky Dates report",
        inputData: {
        ...
        resultData: {
          ...
          support_dates: report.supportDates,
          avoid_dates: report.avoidDates,
          title: report.title,
        },
      });
    } catch (err) {
      ...
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="rounded-[2rem] border border-[#E6C89C]/40 bg-white/65 p-6 shadow-sm backdrop-blur-md sm:p-8">
          ...
          <p className="mt-4 leading-8 text-[#6F5B69]">
            Find your lucky dates based on your date of birth and learn when to
            start work, make decisions, plan meetings, handle money or take
            important steps.
          </p>
          ...
          <div className="mt-6 rounded-[1.5rem] border border-[#E6C89C]/40 bg-[#FFF9F4] p-5">
            ...
            <p className="mt-3 leading-7 text-[#5C3A57]">
              ...
              and map it with compatible date vibrations.
            </p>
          </div>
          ...
          <form onSubmit={handleCalculate} className="mt-6 space-y-5">
            ...
            >
              {saving ? "Calculating..." : "Calculate Lucky Dates"}
            </button>
          ...
          {!result ? (
            <div className="flex h-full min-h-[360px] items-center justify-center rounded-[1.5rem] border border-dashed border-[#E6C89C]/60 bg-[#FFF9F4]/70 p-8 text-center">
              <p className="max-w-md leading-8 text-[#6F5B69]">
                Your complete Lucky Dates report will appear here after
                calculation.
              </p>
            </div>
          ) : (
            <LuckyDatesReportView result={result} />
          )}
        </div>
      </div>
    </section>
...
function LuckyDatesReportView({ result }: { result: LuckyDatesReport }) {
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <ListCard title="Support Dates" items={result.supportDates.map(String)} />
        <ListCard title="Use Carefully" items={result.avoidDates.map(String)} />
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <ListCard title="Best For" items={result.bestFor} />
      </div>
      ...
      <div className="rounded-[1.5rem] bg-[#5C3A57] p-6 text-center text-white">
        <h3 className="font-display text-2xl">Want auspicious date selection?</h3>
        <p className="mt-3 text-sm leading-7 text-[#F6EEE8]">
          Lucky dates from numerology are useful. For marriage, business,
          griha pravesh, travel or puja muhurat, book a personalised date
          consultation.
        </p>
        <Link
          href="/book?service=lucky-dates-consultation"
          className="mt-5 inline-flex rounded-full bg-white px-6 py-3 text-sm font-medium text-[#5C3A57]"
        >
          Book Lucky Dates Consultation
        </Link>
      </div>
    </div>
  );
